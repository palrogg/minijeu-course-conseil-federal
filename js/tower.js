(function() { // private module pattern
  ...
  var FPS           = 60,                                                   // 'update' frame rate fixed at 60fps independent of rendering loop
      ...
      IMAGES        = ['ground', 'ladder', 'player', 'monster', 'coins', 'bundeshaus'],   // sprite image files for loading
  ...
  function run() {
    ...
    Game.Load.images(IMAGES, function(images) {
      Game.Load.json("levels/level1", function(level) {
        ...
        $('#mobilebuttons button').bind('touchstart mousedown', function(e){
          onMobileButton($(this).data('mv'), true);
          e.preventDefault();
        });
        $('#mobilebuttons button').bind('touchend mouseup', function(e){
          onMobileButton($(this).data('mv'), false);
          e.preventDefault();
        });
      });
    });
  ...
  function onkey(ev, key, pressed) {
    switch(key) {
      ...
      case 33: logHeight(); doCheat(); return false;
      case 34: logHeight(); return false;
  ...
  function onMobileButton(movement, pressed){
    ...
    switch(movement){
      ...
      case "up":    player.input.up    = pressed; return false;
      case "down":  player.input.down  = pressed; return false;
        ...
        if(player.score < 1){ // 124
          ...
          if(!playerDidWin){
            ...
            $('#prompt').html(`<h1>Vous avez gagné! </h1>
              ...
              `);
            $('#prompt').show(500, function(){
              console.log('shown')
            });
          }
        }
      }else if(playerLackedOfVoices){
        $('#prompt').hide(500);
        playerLackedOfVoices = false;
      }else{
  ...
  var Renderer = Class.create({
    ...
    initialize: function(images) {
      ...
      this.Bundeshaus    = this.createBundeshaus();
    ...
    render: function(dt) {
      ...
      this.renderBundeshaus(this.ctx);
    ...
    renderBundeshaus: function(ctx) { // skam
      var Bundeshaus = this.Bundeshaus,
          x      = Bundeshaus.w * (camera.rx/tower.w),
          y      = ty(3935);
    //ctx.drawImage(image, sx, sy, sLargeur, sHauteur, dx, dy, dLargeur, dHauteur);
      ctx.drawImage(this.images.bundeshaus, -Bundeshaus.w/2, y, Bundeshaus.w, Bundeshaus.h);
    },
    ...
    renderFront: function(ctx) {
      /* pr edit - no border
      ctx.strokeStyle = 'transparent'; //tower.color.stroke;
      ctx.lineWidth   = 1.2;
      */
      var left   = x2col(camera.rx - tower.w/4),
    ...
    renderScore: function(ctx) {
      if (player.score > this.vscore) {
        this.vscore = this.vscore + 1; // pr edit - vs + 50
